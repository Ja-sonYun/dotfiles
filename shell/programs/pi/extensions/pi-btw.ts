...
function getState(): BtwState {
  const globalState = globalThis as unknown as {
    [k: symbol]: Partial<BtwState> | undefined;
  };
...
function trimMessageForContext(message: Message): Message {
  if (message.role === "assistant") {
    const text =
      ...
      "[assistant message omitted for /btw speed]";
    return { ...message, content: [{ type: "text", text }] };
  }
  if (message.role === "user") {
    return {
      ...message,
      content: [{ type: "text", text: userContentToText(message.content) }],
    };
  }
  const text =
    contentPartsToText(message.content) ||
    "[tool result omitted for /btw speed]";
  return {
    role: "user",
    content: [
      { type: "text", text: `[tool result: ${message.toolName}]\n${text}` },
    ],
...
function selectFullContextMessages(messages: Message[]): Message[] {
  if (
    messages.length <=
    FULL_CONTEXT_HEAD_MESSAGE_LIMIT + FULL_CONTEXT_TAIL_MESSAGE_LIMIT
  ) {
...
function selectContextMessages(
  ...
  return mode === "full"
    ? selectFullContextMessages(messages)
    : selectRecentContextMessages(messages);
}
...
function parseBtwArgs(args: string): ParsedBtwArgs {
  ...
  if (trimmed.startsWith("--recent ")) {
    return {
      mode: "recent",
      question: trimmed.slice("--recent ".length).trim(),
    };
  }
  if (trimmed === "--recent") {
...
function readCurrentContextMessages(
  ctx: ExtensionContext,
  mode: BtwContextMode,
): Message[] {
  ...
  try {
    const response = await completeSimple(
      model,
      {
        ...
        messages: buildBtwMessages(ctx, userMessage, mode),
        tools: [],
      },
      {
        apiKey: auth.apiKey,
...
function registerBtwCommand(pi: ExtensionAPI): void {
  pi.registerCommand(BTW_COMMAND_NAME, {
    description:
      "Ask a by-the-way question without polluting the main conversation",
...
async function handleBtwHistoryCommand(
  ctx: ExtensionCommandContext,
): Promise<void> {
  if (ctx.mode !== "tui") {
    ctx.ui.notify(MSG_REQUIRES_INTERACTIVE, "error");
...
class BtwOverlayController implements Component {
  ...
  constructor(
    ...
    this.startSpinner();
    this.controller.signal.addEventListener("abort", () => this.stopSpinner(), {
      once: true,
    });
  }
  ...
  private renderBanner(width: number): string {
    ...
    const truncatedQuestion = truncateToWidth(
      this.question,
      questionWidth,
      "…",
      false,
  ...
  private renderAnswer(width: number): string[] {
    ...
    if (this.mode === "error") {
      ...
      for (const line of this.error.split("\n")) {
        ...
        out.push(
          ...wrapTextWithAnsi(this.theme.fg("error", source), bodyWidth),
        );
      }
      return indent(out);
    }
