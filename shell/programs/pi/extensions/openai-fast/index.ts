/**
 * Vendored from:
 * https://github.com/diegopetrucci/pi-extensions/blob/main/extensions/openai-fast/index.ts
 */
...
function readConfigFile(path: string): RecursivePartial<OpenAIFastConfig> {
  ...
  try {
    ...
    return isPayloadRecord(parsed)
      ? (parsed as RecursivePartial<OpenAIFastConfig>)
      : {};
  } catch (error) {
...
export default function openAIFastExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("fast", {
    description:
      "Toggle OpenAI Codex Fast mode for ChatGPT-auth GPT-5.4/GPT-5.5",
