...
show_menu() {
	...
	if tmux show-environment DEFAULT >/dev/null 2>&1; then
		"$TMUX_MENU" show --menu "$TMUX_CONFIG"/menu/menu.yaml --working_dir "$pane_current_path" || notify_error "$TMUX_MENU show"
	else
		tmux detach || notify_error "detach"
		W=$(tmux display -p "#{client_width}")
		W=$((W - 1))
		H=$(tmux display -p "#{client_height}")
		"$TMUX_MENU" show -x "$W" -y "$H" --menu "$TMUX_CONFIG"/menu/menu.yaml --working_dir "$pane_current_path" || notify_error "$TMUX_MENU show"
	fi
}
