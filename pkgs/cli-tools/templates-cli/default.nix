...
let
  ...
  completionScript = pkgs.writeText "_templates" ''
    ...
    _templates "$@"
  '';
  command = pkgs.writeScriptBin "templates" ''
    #!${pkgs.zsh}/bin/zsh
    set -euo pipefail

    if [[ "''${1-}" == "-h" || "''${1-}" == "--help" ]]; then
      print -r -- "Manage flake dev environment templates. Subcommands: list, init, info, search, direnv, echo"
      exit 0
    fi
    ...
    case "''${1:-}" in
      ...
      echo)
        ${pkgs.coreutils}/bin/cat "''${FLAKE_TEMPLATES_DIR:?Set FLAKE_TEMPLATES_DIR to the local templates directory}/shells.nix"
        ;;
...
pkgs.symlinkJoin {
  ...
  postBuild = ''
    ...
    cp ${completionScript} "$out/share/zsh/site-functions/_templates"
  '';
}
