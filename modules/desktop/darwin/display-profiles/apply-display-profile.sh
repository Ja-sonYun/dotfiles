...
mapfile -t display_args < <(
    ...
    function find_display_for_profile(profile_index,   display_index, seen, selected) {
      seen = 0
      selected = 0
      ...
      for (display_index = 1; display_index <= display_count; display_index++) {
        ...
        seen++
        if (profile_match_index[profile_index] == seen) {
          return display_index
        }
        selected = display_index
      }

      return profile_match_index[profile_index] == "" && seen == 1 ? selected : 0
    }
    ...
    function try_layout(layout_index,   i, profile_index, display_index, arg) {
      ...
      for (i = 1; i <= layout_profile_count[layout_index]; i++) {
        ...
        if (display_index == 0 || selected_displays[display_index]) {
          return 0
        }
        selected_displays[display_index] = 1
