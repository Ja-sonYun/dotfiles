...
let
  ...
  mkClaudeMcpServers =
    ...
    lib.mapAttrs
      ...
      (
        lib.filterAttrs (
          _: server: !bridgeRemote || lib.hm.mcp.resolveEnabled server != false
        ) cfg.mcp.servers
      );
...
in
{
  ...
  config = lib.mkIf cfg.enable (
    lib.mkMerge [
      ...
      (lib.mkIf config.programs.claude-desktop.enable {
        programs.claude-desktop.settings.mcpServers = claudeDesktopMcpServers;
      })
