      ...
      in
      {
        ...
        claude =
          let
            ...
            pluginName = "hm";
            mcpServers = builtins.filter isDirectMcpServer (keysWith data.mcp "allow");
          ...
          in
          {
            ...
            deny =
              ...
              ++ bashRules "deny"
              ++ map (server: "mcp__plugin_${pluginName}_${server}__*") (
                builtins.filter isDirectMcpServer (keysWith data.mcp "deny")
              )
              ++ lib.optional (data.read == "deny") "Read(**)"
              ++ lib.optional ((data.edit or "ask") == "deny") "Edit(**)"
              ++ lib.optional ((data.write or "ask") == "deny") "Write"
              ++ lib.optional (data.web_search == "deny") "WebSearch"
              ++ lib.optional (data.web_fetch == "deny") "WebFetch(domain:*)";
          };
      };
in
{
  ...
  config = lib.mkIf (cfg.enable && permissions != null) (
    lib.mkMerge [
