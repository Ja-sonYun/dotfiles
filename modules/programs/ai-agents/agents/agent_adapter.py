...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--client", choices=MODEL_CLIENTS, action="append", required=True
    )
    return parser.parse_args()


def read_model_map(path: Path, clients: list[str]) -> ModelMap:
    data = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(data, dict) or set(data) != set(clients):
    ...
    for client in clients:
        ...
        for tier, value in models.items():
            if not isinstance(value, dict):
                raise TypeError(f"Invalid {client} {tier} model config in {path}")
...
def split_frontmatter(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if not isinstance(data, dict):
        raise TypeError(f"Invalid frontmatter in {path}")
...
def string_field(data: dict[str, Any], field: str, path: Path) -> str | None:
    ...
    if not isinstance(value, str):
        raise TypeError(f"{field} must be a string in {path}")
    return value
...
def render_claude(
    ...
    if agent.tools is not None:
        frontmatter["tools"] = [
            f"mcp__plugin_{claude_plugin}_{tool.server}__*" for tool in agent.tools
        ]
...
def pi_proxy_name(server: str) -> str:
    # Match pi-mcp-adapter 2.34.0's formatServerNamespace in types.ts.
    # https://github.com/nicobailon/pi-mcp-adapter/blob/v2.34.0/types.ts
    normalized = server.replace("-", "_")
    safe = re.fullmatch(r"[A-Za-z0-9_]*", normalized) is not None
    safe = safe and not normalized.startswith("_mcpns_")
    body = (
        normalized
        if safe
        else "".join(
            ...
            if character == "_"
                ...
                character
                if re.fullmatch(r"[A-Za-z0-9]", character)
                else f"_{ord(character):x}_"
            )
            for character in normalized
        )
    )
    namespace = body if safe else f"_mcpns_{body}"
    if len(namespace) > 59:
        digest = hashlib.sha256(namespace.encode("ascii")).hexdigest()[:16]
        hash_prefix = "_mcpns__h_"
        head = body[: 59 - len(hash_prefix) - len(digest) - 1]
        namespace = f"{hash_prefix}{head}_{digest}"
    return f"mcp__{namespace}"
...
def render_pi(
    ...
    if agent.permission_mode is not None:
        frontmatter["permissionMode"] = agent.permission_mode
    rendered = yaml.safe_dump(
        frontmatter,
        allow_unicode=True,
        sort_keys=False,
...
def write_outputs(
    ...
    for path in sorted(source_dir.glob("*.md")):
        ...
        if "claude" in model_map:
            render_claude(agent, model_map, output_dir, claude_plugin)
        if "codex" in model_map:
...
def main() -> None:
    ...
    model_map = read_model_map(args.model_map, args.client)
