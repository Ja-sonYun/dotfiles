...
function u.task_cb(fn) -- {{{
	...
	return function(...)
		...
		if is_hstask(out) then
			if out[1] ~= 0 then
				log.e("yabai query failed: " .. out[3])
				return
			end

			-- Replace non-JSON infinity values reported by yabai before decoding.
			local clean = out[2]:gsub(":inf,", ":0,")
			local ok, decoded = pcall(hs.json.decode, clean)
			if not ok or type(decoded) ~= "table" then
				log.e("yabai query returned invalid JSON")
				return
			end
...
function u.getfield(path, tbl, isSafe) -- {{{
	...
	return isSafe and val == nil and res -- return last non-nil value found
...
function u.equal(a, b) -- {{{
	...
	if all_tbls and (u.length(a) ~= u.length(b)) or a == nil or b == nil then
		return false
	end
...
function u.levenshteinDistance(str1, str2) -- {{{
	...
	for i = 1, len1 do
		for j = 1, len2 do
			dist[i][j] =
				math.min(dist[i - 1][j] + 1, dist[i][j - 1] + 1, dist[i - 1][j - 1] + (c1[i] == c2[j] and 0 or 1))
		end
	end
